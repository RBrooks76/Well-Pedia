...
@section('content')
    ...
    <main>
        ...
        <section>
            <div class="section_container container_left_helper container_right_helper">
                <form action="{{ route('onClientForget') }}" method="post" class="">
                    @csrf
                    ...
                    <div class="form_container form_mx_container mb_lg">
                        ...
                        <div class="{{ $errors->has('company_code') ? 'has-error' : '' }} form-group">
                            ...
                            <input type="text" class="form-control" name="company_code" style="{{ $errors->has('company_code') ? 'border : 1px solid #D9214E!important;' : '' }}">
                        ...
                        <div class="{{ $errors->has('email') ? 'has-error' : '' }} form-group">
                            ...
                            <input type="email" class="form-control" name="email"  style="{{ $errors->has('email') ? 'border : 1px solid #D9214E!important;' : '' }}">
                            @if($errors->has('email'))
                                <span class="error">{{ $errors->first('email') }}</span>
                            @endif
                        </div>
                        <div class="form_control_action text-center">
                            <button class="btn_be_action action_dark sm_size action_mt" type="submit">
                                ログイン
                            </button>
                        </div>
